...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const { topic, subject } = body; // Action is implicitly "study_content"
        const topicName = topic?.name || "Tópico desconhecido";

        const prompt = `Crie conteúdo de estudo para "${topicName}".
...
Exemplo: { "content": "## 1. Introdução\nTexto...\n\n## 2. Conceitos\nTexto...", "key_concepts": ["..."], "common_mistakes": ["..."], "exam_tips": ["..."] }`;
        const jsonSchema = { content: "string", key_concepts: [], common_mistakes: [], exam_tips: [] };
