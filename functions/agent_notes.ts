...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const { action, topic, prompt: customPrompt } = body; 
...
Exemplo: { "nodes": [{ "text": "Raiz", "isRoot": true }, { "text": "Filho", "isRoot": false, "children": [] }] }`;
            ...
            // Used for content transformations (deepen, simplify, etc)
            prompt = customPrompt + "\n\nRetorne APENAS JSON válido com chave 'transformed_content'.";
            jsonSchema = { transformed_content: "string" };
        } else {
                 ...
                 children.forEach((child, i) => {
                     ...
                     if (Array.isArray(child.children)) {
                         child.children.forEach((sub, j) => {
                             const subId = `${id}-${j}`;
                             nodes.push({ id: subId, text: sub, x: x + (Math.random() * 60 - 30), y: y + (Math.random() * 60 + 30), color: colors[i % colors.length] });
